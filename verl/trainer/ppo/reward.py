...
def load_reward_manager(
    config: DictConfig, tokenizer: Any, num_examine: int, split: str, **reward_kwargs: Any
    ...
    # For score and comparison rewards
    if 'pair' in reward_manager_name:
        if split == 'train':
            reward_kwargs['n_rollouts'] = config.actor_rollout_ref.rollout.n
            reward_kwargs['reward_score_range'] = config.reward_model.reward_score_range
        else:
            reward_kwargs['n_rollouts'] = 1
            reward_kwargs['reward_score_range'] = config.reward_model.reward_score_range
